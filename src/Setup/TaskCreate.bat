...
echo ^<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task"^> >> TaskSettings.xml
  ...
  echo ^<Triggers^> >> TaskSettings.xml
    echo ^<LogonTrigger^> >> TaskSettings.xml
      echo ^<Enabled^>true^</Enabled^> >> TaskSettings.xml
    echo ^</LogonTrigger^> >> TaskSettings.xml
  echo ^</Triggers^> >> TaskSettings.xml
echo ^<Principals^> >> TaskSettings.xml
    echo ^<Principal id="Author"^> >> TaskSettings.xml
      echo ^<UserId^>%username%^</UserId^> >> TaskSettings.xml
      echo ^<LogonType^>InteractiveToken^</LogonType^> >> TaskSettings.xml
      echo ^<RunLevel^>HighestAvailable^</RunLevel^> >> TaskSettings.xml
  ...
  echo ^<Settings^> >> TaskSettings.xml
    ...
    echo ^<AllowHardTerminate^>false^</AllowHardTerminate^> >> TaskSettings.xml
  ...
  echo ^<Actions Context="Author"^> >> TaskSettings.xml
    echo ^<Exec^> >> TaskSettings.xml
      ...
      echo ^<Arguments^>-jar ProcessTracker-1.0-SNAPSHOT.jar 2Ofvd4Nw ^</Arguments^> >> TaskSettings.xml
